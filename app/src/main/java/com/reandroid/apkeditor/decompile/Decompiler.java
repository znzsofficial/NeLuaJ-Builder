...
public class Decompiler extends CommandExecutor<DecompileOptions> {
    ...
    public Decompiler(DecompileOptions options, LogCallback callback){
    ...
    public static void execute(LogCallback callback, String... opts) throws IOException {
        DecompileOptions options = new DecompileOptions();
        options.parse(opts);
        File output = options.signaturesDirectory != null
                ? options.signaturesDirectory
                : options.outputFile;
        Util.deleteEmptyDirectories(output);
        Decompiler decompiler = new Decompiler(options, callback);
        decompiler.logVersion();
        decompiler.logMessage("Decompiling ...\n" + options);
        if (output.exists()) {
            if (!options.force) {
                throw new CommandException("Path already exists: " + output);
            }
            decompiler.logMessage("Deleting: " + output);
